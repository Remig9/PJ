...
export default ({ navigation }) => {
  ...
  const [showToppingModalPJ, setShowToppingModalPJ] = useState(false);
  const [selectedToppings, setSelectedToppings] = useState([]);
  const [selectedToppingsPJ, setSelectedToppingsPJ] = useState([]);

  const onToggleTopping = (selectedToppings) => {
    setSelectedToppings(selectedToppings);
  };
  const onToggleToppingPJ = (selectedToppings) => {
    setSelectedToppingsPJ(selectedToppings);
  };
  ...
  const addToCartHandler = () => {
    ...
    let toppingPrice = 0;
    if (isPizzaJungle()) {
      if (selectedToppingsPJ.length > 0 && selectedToppings.length === 3) {
        toppingPrice = selectedToppingsPJ
          .map((topping) => +topping.price)
          .reduce((a, b) => a + b);
      }
      allSelectedToppings = [...selectedToppings, ...selectedToppingsPJ];
    } else {
      toppingPrice =
  ...
  const isPizzaJungle = () => item.name.toLowerCase() === "my pizza jungle";
  ...
  return (
    <>
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={s.container}>
          ...
          <View style={s.mainSection}>
            ...
            <AddedToppings
              ...
              heading={
                isPizzaJungle() ? "Free Toppings Added" : "Toppings added"
              }
            />

            {isPizzaJungle() && (
              <AddedToppings
                heading="Additional Toppings added"
                toppings={selectedToppingsPJ}
              />
            )}
            ...
            <CustomButton
              ...
              title={isPizzaJungle() ? "Add 3 free toppings" : "Add Topping(s)"}
              onPress={() => setShowToppingModal(true)}
            />

            {isPizzaJungle() && selectedToppings.length === 3 && (
              <CustomButton
                style={s.addTopping}
                textStyle={{ color: "white" }}
                title="Add additional Toppings"
                onPress={() => setShowToppingModalPJ(true)}
              />
            )}
            ...
            <View style={s.cartButtons}>
              <CustomButton
                disabled={
                  !quantity || (isPizzaJungle() && selectedToppings.length < 3)
                }
                style={s.addToCart}
                textStyle={{ color: "white" }}
                title="add to cart"
      ...
      <AddToppingModal
        ...
        maxToppings={isPizzaJungle() ? 3 : null}
      ...
      <AddToppingModal
        showModal={showToppingModalPJ}
        onCloseModal={() => setShowToppingModalPJ(false)}
        onToggleTopping={onToggleToppingPJ}
      />
